...
@Component
@RequestScope
public class SearchM33CompanyRefundSuccessListWsProviderImpl extends WebServiceBase<SearchM33PaybackListRequest, SearchResponse> {
	private static final Logger log = LoggerFactory.getLogger(SearchM33CompanyRefundSuccessListWsProviderImpl.class);
	...
	public SearchM33CompanyRefundSuccessListWsProviderImpl(MessageHandler messageHandler) {
	...
	protected ResponseBean validate(SearchM33PaybackListRequest request) throws Exception {
		...
		if (NumberUtils.isZeroOrNull(request.getCompanyId()) && StringUtils.isEmpty(request.getReceiveNo())) {
	...
	protected SearchResponse implement(SearchM33PaybackListRequest request) throws Exception {
		timer.start("SearchM33CompanyRefundSuccessListWsProviderImpl");
		...
		response.setResult(paybackM33Service.searchCompanyRefundSuccessList(request, request.getPaginable()));
		response.setStatus(ResponseStatus.SUCCESS);
		log.debug(timer.endToString("SearchM33CompanyRefundSuccessListWsProviderImpl"));
		return response;
	}
}
