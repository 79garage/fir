...
@Component
@RequestScope
public class SearchM33CompanyPaybackListWsProviderImpl extends WebServiceBase<SearchM33PaybackListRequest, SearchResponse> {
	...
	public SearchM33CompanyPaybackListWsProviderImpl(MessageHandler messageHandler) {
	...
	protected ResponseBean validate(SearchM33PaybackListRequest request) throws Exception {
		...
		if (NumberUtils.isZeroOrNull(request.getCompanyId()) && StringUtils.isEmpty(request.getReceiveNo()) && StringUtils.isEmpty(request.getPaybackNo())) {
	...
	protected SearchResponse implement(SearchM33PaybackListRequest request) throws Exception {
		timer.start("SearchM33CompanyPaybackListWsProviderImpl");
		...
		response.setResult(paybackM33Service.searchCompanyPaybackList(request, request.getPaginable()));
		response.setStatus(ResponseStatus.SUCCESS);
		log.debug(timer.endToString("SearchM33CompanyPaybackListWsProviderImpl"));
		return response;
	}
}
