...
@Component
@RequestScope
public class SearchPaybackEmployeeBatchWsProviderImpl extends WebServiceBase<SearchPaybackEmployeeBatchRequest, SearchResponse> {
    @Autowired
    private ConTbPaybackRequestServiceImpl conTbPaybackRequestService;
    @Autowired
    public SearchPaybackEmployeeBatchWsProviderImpl(MessageHandler messageHandler) {
        super(messageHandler);
    }

    @Override
    protected ResponseBean validate(SearchPaybackEmployeeBatchRequest request) throws Exception {
        if (request == null) {
    ...
    @Override
    protected SearchResponse implement(SearchPaybackEmployeeBatchRequest request) throws Exception {
        List<ConTbPaybackBean> paybackEmployeeList = conTbPaybackRequestService.searchPaybackEmployeeBatch(request.getCreateBy()
                , request.getCreateDate()
                , request.getPaginable()
                , getUser()
                , request.getPaybackRequestIds());
        SearchResponse response = new SearchResponse();
        response.setResult(paybackEmployeeList);
