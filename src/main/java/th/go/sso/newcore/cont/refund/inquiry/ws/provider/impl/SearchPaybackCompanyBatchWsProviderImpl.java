...
@Component
@RequestScope
public class SearchPaybackCompanyBatchWsProviderImpl extends WebServiceBase<SearchPaybackCompanyBatchRequest, SearchResponse> {
    @Autowired
    private ConTbPaybackRequestServiceImpl conTbPaybackRequestService;
    @Autowired
    public SearchPaybackCompanyBatchWsProviderImpl(MessageHandler messageHandler) {
        super(messageHandler);
    }

    @Override
    protected ResponseBean validate(SearchPaybackCompanyBatchRequest request) throws Exception {
        if (request == null) {
    ...
    @Override
    protected SearchResponse implement(SearchPaybackCompanyBatchRequest request) throws Exception {
        List<ConTbPaybackBean> paybackCompanyList = conTbPaybackRequestService.searchPaybackCompanyBatch(request.getCreateBy()
                , request.getCreateDate()
                , request.getPaginable()
                , getUser()
                , request.getPaybackRequestIds());
        SearchResponse response = new SearchResponse();
        response.setResult(paybackCompanyList);
