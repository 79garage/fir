...
@Component
@RequestScope
public class SearchM33EmployeeRefundSuccessListWsProviderImpl extends WebServiceBase<SearchM33PaybackListRequest, SearchResponse> {
	private static final Logger log = LoggerFactory.getLogger(SearchM33EmployeeRefundSuccessListWsProviderImpl.class);
	...
	public SearchM33EmployeeRefundSuccessListWsProviderImpl(MessageHandler messageHandler) {
	...
	protected ResponseBean validate(SearchM33PaybackListRequest request) throws Exception {
		...
		if (NumberUtils.isZeroOrNull(request.getCompanyId()) && StringUtils.isEmpty(request.getIdCardNo()) && StringUtils.isEmpty(request.getReceiveNo())) {
	...
	protected SearchResponse implement(SearchM33PaybackListRequest request) throws Exception {
		timer.start("SearchM33EmployeeRefundSuccessListWsProviderImpl");
		...
		response.setResult(paybackM33Service.searchEmployeeRefundSuccessList(request, request.getPaginable()));
		response.setStatus(ResponseStatus.SUCCESS);
		log.debug(timer.endToString("SearchM33EmployeeRefundSuccessListWsProviderImpl"));
		return response;
	}
}
