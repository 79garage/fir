...
@Component
@RequestScope
public class SearchM39RefundSuccessListWsProviderImpl extends WebServiceBase<SearchM39PaybackListRequest, SearchResponse> {
	private static final Logger log = LoggerFactory.getLogger(SearchM39RefundSuccessListWsProviderImpl.class);
	...
	public SearchM39RefundSuccessListWsProviderImpl(MessageHandler messageHandler) {
	...
	protected ResponseBean validate(SearchM39PaybackListRequest request) throws Exception {
		...
		if (StringUtils.isEmpty(request.getIdCardNo()) && StringUtils.isEmpty(request.getReceiveNo())) {
	...
	protected SearchResponse implement(SearchM39PaybackListRequest request) throws Exception {
		timer.start("SearchM39RefundSuccessListWsProviderImpl");
		...
		response.setResult(paybackM39Service.searchInsurerRefundSuccessList(request, request.getPaginable()));
		response.setStatus(ResponseStatus.SUCCESS);
		log.debug(timer.endToString("SearchM39RefundSuccessListWsProviderImpl"));
		return response;
	}
}
